package com.epam.reportportal.extension.gitlab.info.impl;
  ...
  private void loadBinaryDataInfo(IntegrationType integrationType) {
    ...
    if (ofNullable(details.get(BINARY_DATA_KEY)).isEmpty()) {
      try (InputStream propertiesStream = Files.newInputStream(
        ...
        Map<String, String> binaryDataInfo = binaryDataProperties.entrySet().stream()
            .collect(HashMap::new, (map, entry) -> map.put(String.valueOf(entry.getKey()),
